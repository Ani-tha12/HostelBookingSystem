...
public class UserService {
	...
	public UserResponse registerUser(UserRequest request) {
		...
		try {
			...
			logger.info("User registered successfully with ID: {} and email: {}", savedUser.getUserId(),
					savedUser.getEmail());
	...
	public LoginResponse login(LoginRequest request) {
		...
		try {
			...
			if (user.getStatus() != UserStatus.ACTIVE && user.getStatus() != UserStatus.APPROVED) {
				logger.warn("Login failed: Account not active for email: {} - Status: {}", request.getEmail(),
						user.getStatus());
				throw new UnauthorizedException("Account is not active. Status: " + user.getStatus());
			}

			String token = jwtService.generateToken(user.getEmail(), user.getRole().name());
			logger.info("Login successful for user: {} (ID: {}) with role: {}", user.getEmail(), user.getUserId(),
					user.getRole());
	...
	public List<UserResponse> getAllUsers() {
		...
		List<UserResponse> users = userRepository.findAll().stream().map(userMapper::toResponse)
	...
	public UserResponse approveOwner(Long userId) {
		...
		if (user.getRole() != UserRole.OWNER) {
			logger.warn("Owner approval failed: User is not an owner - ID: {}, Role: {}", userId, user.getRole());
	...
	public void deleteUser(Long userId) {
		...
		if (user.getBookings() != null && !user.getBookings().isEmpty()) {
			logger.warn("Delete failed: User has {} active bookings - ID: {}", user.getBookings().size(), userId);
	...
	public List<UserResponse> getUsersByRole(UserRole role) {
		...
		List<UserResponse> users = userRepository.findAll().stream().filter(user -> user.getRole() == role)
	...
	public String forgotPassword(ForgotPasswordRequest request) {
		...
		User user = userRepository.findByEmailAndName(request.getEmail(), request.getUsername()).orElseThrow(() -> {
			logger.error("User not found with email: {} and username: {}", request.getEmail(), request.getUsername());
			return new ResourceNotFoundException("User not found with provided credentials");
		});
		...
		logger.info("Reset token generated for user: {}", user.getEmail());
	...
	public void resetPassword(ResetPasswordRequest request) {
		...
		User user = userRepository.findByResetToken(request.getResetToken()).orElseThrow(() -> {
		...
		if (!user.getName().equals(request.getUsername())) {
			logger.error("Username doesn't match for reset token - Expected: {}, Got: {}", user.getName(),
					request.getUsername());
			throw new UnauthorizedException("Invalid username");
		}

		if (user.getResetTokenExpiry().isBefore(LocalDateTime.now())) {
			logger.error("Reset token expired for user: {}", user.getEmail());
	...
	public boolean verifyResetToken(String token) {
		...
		if (userOpt.isEmpty()) {
			logger.error("Reset token not found: {}", token);
			return false;
		}
