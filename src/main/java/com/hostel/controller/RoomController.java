...
public class RoomController {
	...
	public ResponseEntity<ApiResponse<RoomResponse>> addRoom(@Valid @RequestBody RoomRequest request) {
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success("Room added successfully", response));
	}
	...
	public ResponseEntity<ApiResponse<List<RoomResponse>>> getAvailableRoomsByHostel(@PathVariable Long hostelId) {
		List<RoomResponse> rooms = roomService.getAvailableRoomsByHostel(hostelId);
		return ResponseEntity.ok(ApiResponse.success("Available rooms for hostel retrieved", rooms));
	}
	...
	public ResponseEntity<ApiResponse<RoomResponse>> updateAvailability(@PathVariable Long roomId,
			@RequestParam Integer availableBeds) {
		RoomResponse response = roomService.updateAvailability(roomId, availableBeds);
		return ResponseEntity.ok(ApiResponse.success("Room availability updated", response));
	}
